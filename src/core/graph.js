...
export function graphIndexEntryFromNode(node) {
  return {
    ...
    scope_paths: asArray(node.scope_paths),
...
function formatGraphNode(cwd, node) {
  ...
  return {
    ...
    source_path: node.data.source_path || node.data.provenance?.source_path || "",
    scope_paths: asArray(node.data.scope_paths || node.data.provenance?.scope_paths),
