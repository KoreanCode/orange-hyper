...
3. `npx` exact-version fallback: 임시 확인이 필요할 때만 `orange-hyper@1.1.0-beta.1`처럼 정확한 버전이나 `@beta`를 지정한다.
...
현재 권장 테스트 채널은 `v1.1.0-beta.1`입니다. 이 빌드는 공식 Closed Beta prerelease이며 npm `latest` stable 채널이 아닙니다.
...
1. standalone `v1.1.0-beta.1`을 user-local 위치에 설치한다.
...
npm fallback은 내가 명시적으로 요청한 경우에만 사용하고, 그때도 `orange-hyper@beta` 또는 `orange-hyper@1.1.0-beta.1`처럼 tag나 정확한 버전을 지정해줘.
