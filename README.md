...
3. `npx` exact-version fallback: 임시 확인이 필요할 때만 `orange-hyper@1.1.0-beta.2`처럼 정확한 버전이나 `@beta`를 지정한다.
...
현재 권장 테스트 채널은 `v1.1.0-beta.2`입니다. 이 빌드는 공식 Closed Beta prerelease이며 npm `latest` stable 채널이 아닙니다.

Primary validated 환경은 macOS arm64, Codex CLI, standalone binary, user-scoped Codex Host Binding, project-scoped Activation, interactive Codex `/hooks` review입니다. macOS x64, Linux x64, Windows x64, 다른 Codex minor version은 실제 사용자 검증이 쌓이기 전까지 exploratory 환경입니다.

이번 Beta에서 사용자는 Orange를 직접 운영하는 대신 평소처럼 AI에게 작업을 요청합니다. AI는 Codex Host Binding을 사용자 환경에 한 번 설치하고, Orange를 사용할 repository만 별도 활성화합니다. Codex `/plugins`에서 plugin을 설치·enable하고 `/hooks`에서 현재 definition을 review하면 lifecycle이 연결됩니다. L0/L1 작업은 조용히 통과하고, L2 이상 작업은 Quest, Capsule, verification evidence를 정책 범위 안에서 관리합니다. 검증 evidence가 관찰되지 않으면 Stop continuation을 한 번만 요청합니다. Working memory와 pending Memory Proposal은 만들 수 있지만 durable memory accept는 자동화하지 않습니다.

Beta 빠른 시작:

1. standalone `v1.1.0-beta.2`을 user-local 위치에 설치한다.
2. Codex Host Binding을 설치한다.
3. Codex `/plugins`에서 Orange plugin을 설치하고 enable한다.
4. Codex `/hooks`에서 current definition을 review한다.
5. 사용할 repository에서 project activation을 적용한다.
6. `orange activate status --host codex --json`으로 `active` 상태를 확인한다.
7. 이후에는 평소처럼 AI에게 작업을 요청한다.

긴 명령과 상태 해석은 [Closed Beta Program](docs/28_CLOSED_BETA_PROGRAM.md), [Activation Runtime](docs/26_ACTIVATION_RUNTIME.md), [Codex Binding E2E Checklist](docs/27_CODEX_BINDING_E2E.md)에 둔다.

자동화되지 않는 안전 경계:

- Memory Proposal accept
- MCP 설치 또는 실행
- project-specific Skill/Agent 생성
- subagent 실행
- branch/PR/SPEC workflow
- telemetry 또는 network upload
- raw prompt와 transcript 저장

Beta 참여자는 [Closed Beta Program](docs/28_CLOSED_BETA_PROGRAM.md), [Beta Test Checklist](docs/29_BETA_TEST_CHECKLIST.md), [Beta Bug](.github/ISSUE_TEMPLATE/beta-bug.yml), [Beta Feedback](.github/ISSUE_TEMPLATE/beta-feedback.yml), [Codex Binding E2E Checklist](docs/27_CODEX_BINDING_E2E.md)를 사용합니다.

alpha.8 사용자는 beta.1 설치 후 plugin version과 binding fingerprint가 바뀌므로 Codex `/hooks`에서 hook definition을 다시 review해야 할 수 있습니다. 기존 accepted project memory는 자동 삭제하거나 초기화하지 않습니다. alpha.8은 재발행하지 않으며, beta.1이 새 Closed Beta 배포 채널입니다.
...
npm fallback은 내가 명시적으로 요청한 경우에만 사용하고, 그때도 `orange-hyper@beta` 또는 `orange-hyper@1.1.0-beta.2`처럼 tag나 정확한 버전을 지정해줘.
...
현재 Identity HTML은 read-only full-screen Knowledge Graph Dashboard를 제공합니다. 첫 화면은 generated Structure Graph와 accepted memory를 합성한 Canvas graph stage이며, floating action dock, search popover, selected-node badge, minimap, click-to-inspect node drawer를 제공합니다. 기본 보기인 Combined 외에 Structure와 Memory 모드를 선택할 수 있습니다. layout은 build 시 결정되어 같은 revision은 같은 좌표를 갖고, search/view filter와 fit/reset/pan/zoom은 source state를 수정하지 않습니다. keyword concept node를 기본으로 만들지 않고, pending/rejected proposal도 포함하지 않습니다. Graph editor가 아니고, Obsidian/JSON Canvas export는 future interoperability layer이며 기본 제품 경험이 아닙니다.
