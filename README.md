...
설치 또는 PATH 확인 뒤에는 `orange activate plan --host codex --scope project --json`으로 read-only activation 계획을 보여줘.
...
AI: `orange activate plan --host codex --scope project --json`으로 설치, 초기화, Codex binding, hook trust, planned paths를 먼저 확인하겠습니다. 승인하면 `apply`를 실행하고, Codex plugin install/trust가 남아 있으면 `pending_trust`로 보고하겠습니다. 실제 lifecycle heartbeat 전에는 active라고 표시하지 않겠습니다.
