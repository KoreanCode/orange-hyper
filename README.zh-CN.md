...
3. `npx` exact-version fallback：只用于临时确认，并指定 `orange-hyper@1.1.0-alpha.7` 或 `@alpha`。
...
只有在我明确要求时才使用 npm fallback；使用时也要指定 `orange-hyper@alpha` 或 `orange-hyper@1.1.0-alpha.7`。

安装或 PATH 确认之后，请先运行 `orange activate plan --host codex --scope project --json`，把 read-only activation plan 给我看。
