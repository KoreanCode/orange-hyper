...
test("init creates the v0.2 storage structure", () => {
  ...
  assert.match(currentCapsule, new RegExp(`Generated by: Orange Hyper ${escapeRegExp(ORANGE_HYPER_VERSION)}`));
...
test("quest lifecycle writes markdown frontmatter, capsule, completion evidence, and doctor passes", () => {
  ...
  assert.match(capsule.content, new RegExp(`Generated by: Orange Hyper ${escapeRegExp(ORANGE_HYPER_VERSION)}`));
...
test("identity summary includes memory proposal and accepted node counts", async () => {
  ...
  assert.match(html, /Memory Mapping/);
  assert.match(html, /Accepted memory rows/);
  assert.match(html, /<meta name="source-repository" content="https:\/\/github\.com\/KoreanCode\/orange-hyper">/);
  assert.match(html, /remember accepted decision/);
...
test("identity build writes generated placeholder html", async () => {
  ...
  assert.match(html, new RegExp(escapeRegExp(config.project_id)));
  ...
  assert.match(html, /graph-view-mode/);
  assert.match(html, /Structure<\/option>/);
  assert.match(html, /Memory<\/option>/);
  assert.match(html, /Combined<\/option>/);
  assert.match(html, /Accepted memory nodes are candidate project memory\./);
  assert.match(html, /Structure Nodes/);
  assert.match(html, /Accepted Memory/);
  assert.match(html, /node-detail-drawer/);
  assert.match(html, /identity-sidebar/);
  assert.match(html, /<meta name="source-repository" content="https:\/\/github\.com\/KoreanCode\/orange-hyper">/);
  assert.match(html, /orange-knowledge-graph-state/);
  assert.doesNotMatch(html, /orange-hyper-state/);
});
...
test("identity build --json prints generated html path and summary", async () => {
  ...
  assert.equal(payload.data.summary.growthPreview.growthLevel, "seed");
...
test("all JSON success envelopes include contract version and dot command ids", () => {
  ...
  assertJsonCommand(runOrange(["graph", "rebuild-index", "--json"], cwd), "graph.rebuildIndex");
  assertJsonCommand(runOrange(["mcp", "list", "--json"], cwd), "mcp.list");
  assertJsonCommand(runOrange(["mcp", "show", "context7", "--json"], cwd), "mcp.show");
  assertJsonCommand(
    runOrange(["mcp", "suggest", "--query", "Spring Security 최신 문서 확인", "--json"], cwd),
    "mcp.suggest"
  );
