...
test("hook run returns warning and hint for missing project_id without repair", () => {
  ...
  assert.ok(payload.data.warnings.some((item) => item.code === "CONFIG_PROJECT_ID_MISSING"));
...
test("--write-report creates only a local hook report under hooks/reports", () => {
  ...
  assert.equal(report.project_boundary.project_id, readConfig(cwd).project_id);
});
