...
v1.1.0-alpha.4는 Project Sync와 Identity Graph Foundation을 추가한다. Identity
...
5. keyword concept 확장은 저품질 token node를 만들 수 있으므로 alpha.4 기본값이 아니다.
```
...
v1.1 alpha.4 visual node type 후보:
...
초기 alpha.4에서는 keyword concept 확장을 비활성화한다. `growthSignal`과
...
v1.1.0-alpha.4 acceptance criteria:
...
- accepted memory mapped by scope path or unmapped-memory cluster
- hamburger sidebar
- node detail drawer
- search/filter drawer
