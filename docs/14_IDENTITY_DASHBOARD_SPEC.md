...
v1.1.0-alpha.3은 이 spec을 구현한다. Identity HTML의 first screen은
`100vw` x `100vh` full-screen dark SVG graph stage가 되며, document-style
report 정보는 sidebar/drawer/fallback으로 내려간다. HTML state는
`sourceGraph`와 `visualGraph`를 분리하고, visualGraph 안에만 derived concept,
source quest, source proposal, category node를 둔다.
...
v1.1.0-alpha.3 acceptance criteria:
...
sourceGraph:
  ...
  source of truth
visualGraph:
  nodes: source memory nodes plus visual-only derived nodes
