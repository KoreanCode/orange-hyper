...
후속 범위다. v0.3.0 stable은 accepted node 탐색, 필터링, deterministic search,
read-only identity preview까지만 안정화한다.
...
orange remember revise <proposal-id> --candidate "..."
orange remember revise <proposal-id> --why "..."
orange remember revise <proposal-id> --confidence low|medium|high
...
v0.3.0 stable에서 shared memory state는 fresh clone에서도 accepted graph
...
```text
propose -> show -> validate -> revise -> validate -> accept|reject
```

`validate`는 pending, accepted, rejected proposal 모두에서 실행할 수 있다.
frontmatter, 필수 섹션, quality validation, source Quest 존재 여부를 검사한다.

`revise`는 pending proposal에만 허용한다. 수정 가능 범위는 다음으로 제한한다.

- `Candidate Memory`
- `Why this should be remembered`
- frontmatter `confidence`와 `Suggested Node`의 confidence display

`revise`는 저장 전 validation을 수행하고, 저장 후 `updated_at`을 갱신한 proposal을
다시 validation한다. 수정 결과가 다른 pending proposal의 `Candidate Memory`와
중복되면 실패한다. accepted/rejected proposal은 사용자의 과거 결정 기록이므로
수정하지 않는다.
...
v0.3.0 stable은 accepted memory node 탐색과 read-only preview만 제공한다.
...
```text
1. Agent proposes memory delta from a completed Quest
2. User or adapter reviews the pending proposal
3. User or adapter validates and optionally revises the pending proposal
4. User accepts or rejects the proposal
5. Accepted delta becomes a candidate graph node with provenance
6. Future retrieval can use accepted candidate nodes when retrieval exists
```
