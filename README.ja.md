...
v1.0.0 stable は Orange Hyper の first stable boundary release です。新しい feature release ではなく、v0.1 から v0.8 で検証した Seed Kernel、Memory Delta Proposal、Memory Graph Usability、read-only Identity Graph Preview、Minimal Hook Preview、MCP Advisor、Growth Signal Preview、Adapter Invocation Contract、local-only Eval and Reports の境界を stable surface として固定します。
...
- `eval snapshot`、`eval report`、`eval explain` は `.orange-hyper` local project state だけを読み、Quest、verification、proposal、graph、doctor、hook report、MCP Advisor、growth、adapter、identity の信号を保守的に要約します。
- eval report は summary、section `status`、`reason`、`evidence_count`、unavailable metric、known gap を JSON/Markdown に出します。
- eval report はデフォルトでは stdout だけを使います。`--write-report` を明示した場合だけ、`.orange-hyper/evals/reports/` に Markdown report を作ります。
- Eval and Reports stable は外部 telemetry、network upload、LLM judge、token savings estimate、success-rate improvement claim、MCP 実行、hook 自動実行、project memory/config 自動変更を行いません。

## Command Surface

v1.0.0 stable audit の CLI command surface は次の通りです。
