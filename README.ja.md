...
3. `npx` exact-version fallback: 一時確認だけに使い、`orange-hyper@1.1.0-beta.1` または `@beta` を指定します。
...
現在の推奨テストチャネルは `v1.1.0-beta.1` です。この build は公式 Closed Beta prerelease であり、npm `latest` stable チャネルではありません。
...
1. standalone `v1.1.0-beta.1` を user-local の場所にインストールします。
...
npm fallback は私が明示的に求めた場合だけ使い、その場合も `orange-hyper@beta` または `orange-hyper@1.1.0-beta.1` を指定してください。
