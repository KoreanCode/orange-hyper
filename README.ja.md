...
v0.5.0-alpha.1 時点で、Orange Hyper は Seed Kernel、Memory Graph Usability、read-only Identity Graph Preview、Minimal Hook Preview、MCP Advisor alpha 機能を提供します。
...
- `mcp list`、`mcp show`、`mcp suggest` は score と no-suggestion 状態を持つ read-only MCP proposal card だけを提案します。
- MCP Advisor は MCP をインストールまたは実行せず、config や project memory も変更しません。
